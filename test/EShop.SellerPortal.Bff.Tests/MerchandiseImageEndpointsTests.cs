// <copyright file="MerchandiseImageEndpointsTests.cs" company="Henrik Jensen">
...
public sealed class MerchandiseImageEndpointsTests : IAsyncLifetime
{
    ...
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-upload");
    ...
    [Fact]
    public async Task PostImage_Twice_AddsBothInsteadOfReplacing()
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-add");
        ...
        getFirst.EnsureSuccessStatusCode();
    }

    [Fact]
    public async Task PostImage_WhenAlreadyAtThreeImages_ReturnsBadRequest()
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-full");
        await PostImageAsync(client, draftId, [1], "image/png");
        await PostImageAsync(client, draftId, [2], "image/png");
        await PostImageAsync(client, draftId, [3], "image/png");

        HttpResponseMessage response = await PostImageAsync(client, draftId, [4], "image/png");
    ...
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-badtype");
    ...
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-toolarge");
    ...
    {
        (HttpClient authenticatedClient, Guid draftId) = await CreateDraftAsync("seller-merch-image-noantiforgery");
        using HttpClient client = factory.CreateClient();
        client.DefaultRequestHeaders.Add(TestAuthHandler.UserHeaderName, "seller-merch-image-noantiforgery");
    ...
    {
        (HttpClient client, Guid draftId) = await CreateDraftAsync("seller-merch-image-delete");
        ...
        HttpResponseMessage deleteResponse = await client.DeleteAsync(
            new Uri($"/bff/api/drafts/merchandise/{draftId}/images/{image.Id}", UriKind.Relative), TestContext.Current.CancellationToken);
    ...
    private static async Task<HttpResponseMessage> PostImageAsync(HttpClient client, Guid draftId, byte[] content, string contentType)
    {
        ...
        return await client.PostAsync(
            new Uri($"/bff/api/drafts/merchandise/{draftId}/images", UriKind.Relative), form, TestContext.Current.CancellationToken);
    }
}
