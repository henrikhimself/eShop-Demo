...
// Same test double as EShop.SellerPortal.Bff.Tests' RecordingServiceBusClient - kept as
// a local copy rather than a shared package: both types it subclasses expose a
// protected parameterless constructor specifically to support this kind of lightweight
// subclass, with no mocking library or Service Bus emulator needed.
internal sealed class RecordingServiceBusClient : ServiceBusClient
{
    ...
    {
        return new RecordingServiceBusSender(SentMessages);
    }
}

internal sealed class RecordingServiceBusSender(List<ServiceBusMessage> sentMessages) : ServiceBusSender
{
