...
// Shared consumer base class lives in EShop.Messaging so EShop.DevTools can reuse it without depending on the Bff.
    ...
    ILogger logger) : BackgroundService
{
    ...
    {
        // AutoCompleteMessages defaults to true, which would race the explicit
        // CompleteMessageAsync/DeadLetterMessageAsync calls below - disabled since this
        // class always completes or dead-letters a message itself.
        _processor = client.CreateProcessor(queueName, new ServiceBusProcessorOptions { AutoCompleteMessages = false });
        _processor.ProcessMessageAsync += ProcessMessageAsync;
        _processor.ProcessErrorAsync += ProcessErrorAsync;

        // Retries a not-yet-reachable Service Bus namespace instead of crashing the whole host).
    ...
    // Public, not protected: lets a test project call a concrete consumer's handling
    // logic directly, without needing to construct a real ServiceBusProcessor's
    // ProcessMessageEventArgs (which requires a live/emulated broker connection).
    ...
    private async Task ProcessMessageAsync(ProcessMessageEventArgs args)
    {
        ...
        {
            // The target record doesn't exist and never will, so dead-letter for manual
            // reconciliation instead of retrying via ProcessErrorAsync's redelivery.
