// <copyright file="InventoryNotificationBroadcaster.cs" company="Henrik Jensen">
...
internal sealed class InventoryNotificationBroadcaster
{
    private readonly ConcurrentDictionary<Guid, List<Channel<InventorySummary>>> _subscribersBySellerId = new();

    public Channel<InventorySummary> Subscribe(Guid sellerId)
    {
        var channel = Channel.CreateUnbounded<InventorySummary>();

        while (true)
        {
            List<Channel<InventorySummary>> subscribers = _subscribersBySellerId.GetOrAdd(sellerId, static _ => []);
            lock (subscribers)
            {
                ...
                if (!_subscribersBySellerId.TryGetValue(sellerId, out List<Channel<InventorySummary>>? current)
    ...
    public void Unsubscribe(Guid sellerId, Channel<InventorySummary> channel)
    {
        if (!_subscribersBySellerId.TryGetValue(sellerId, out List<Channel<InventorySummary>>? subscribers))
        {
            return;
        }
    ...
    public void Publish(Guid sellerId, InventorySummary summary)
    {
        if (!_subscribersBySellerId.TryGetValue(sellerId, out List<Channel<InventorySummary>>? subscribers))
        {
            return;
        ...
        Channel<InventorySummary>[] snapshot;
        lock (subscribers)
        {
            snapshot = [.. subscribers];
        }

        foreach (Channel<InventorySummary> channel in snapshot)
        {
