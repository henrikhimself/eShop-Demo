...
// Moves the seller-portal cookie's auth ticket out of the cookie itself and into
// HybridCache (L1 in-process memory, L2 kv store) - the cookie then only holds this
// store's opaque key, never approaching the size ChunkingCookieManager splits at.
internal sealed class HybridCacheTicketStore(HybridCache cache) : ITicketStore
{
    private const string KeyPrefix = "seller-portal-ticket:";
