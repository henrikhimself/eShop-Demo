...
internal static class DbContextConfiguration
{
    ...
    public static void AddDbContextConfiguration(this WebApplicationBuilder builder, bool isBuildTimeOpenApiGeneration)
    {
        ...
        {
            // Minimal API's binding inference needs each handler parameter to resolve
            // as *some* service or it treats it as a request body (breaks GETs) - this
            // fake only needs to exist, never connect to anything real.
            builder.Services.AddDbContext<SellerPortalDbContext>(options =>
                options.UseSqlServer("Server=fake;Database=fake;TrustServerCertificate=True;"));
        }
    }
}
