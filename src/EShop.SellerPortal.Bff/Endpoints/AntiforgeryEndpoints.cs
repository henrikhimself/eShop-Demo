...
internal static class AntiforgeryEndpoints
{
    ...
    // Standard ASP.NET Core SPA pattern: the Next.js app reads this non-HttpOnly cookie
    // and echoes its value back as the X-XSRF-TOKEN header on every mutating request
    // (see lib/bff-fetch.ts). GetAndStoreTokens also sets antiforgery's own HttpOnly
    // verification cookie as a side effect. Secure mirrors the request scheme, matching
    // the auth cookie's own SameAsRequest policy for local HTTP dev (see Program.cs).
