// <copyright file="InventoryEventsEndpoints.cs" company="Henrik Jensen">
...
internal static class InventoryEventsEndpoints
{
    ...
    public static IEndpointRouteBuilder MapInventoryEventsEndpoints(this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet("/bff/api/inventory/events", async (
            HttpContext context,
            ...
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    if (completed == waitToRead)
                    {
                        ...
                        if (hasData)
                        {
                            while (channel.Reader.TryRead(out InventorySummary? summary))
                            {
                                if (summary is null)
                                {
                                    continue;
        ...
        // Server-Sent Events - OpenAPI has no SSE representation, and InventorySummary's
        // schema is already covered by GET /bff/api/inventory (see
