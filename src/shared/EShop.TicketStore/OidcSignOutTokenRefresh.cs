...
// See doc/CHRONICLE.md — Keycloak restarts can invalidate id_token_hint without changing its exp claim, so Keycloak mode force-confirms with the provider instead of trusting IsExpired alone.
// Shared by EShop.SellerPortal.Bff and EShop.StoreFront.Web.
public static class OidcSignOutTokenRefresh
{
...
// StillValid needs no caller action. NothingToValidate: no id_token to check - callers leave whatever they had alone.
// Refreshed carries new tokens for the caller to persist. Rejected: neither the id_token nor a refresh attempt produced anything usable.
...
public enum RefreshOutcomeKind
{
    NothingToValidate,
    StillValid,
    Refreshed,
...
public sealed record RefreshedTokens(string IdToken, string? RefreshToken);
