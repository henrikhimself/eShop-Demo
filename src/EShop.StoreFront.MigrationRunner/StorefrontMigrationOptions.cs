...
// Everything the migrator needs for one component. ToolsDirectory/BaselineScriptFileName/
// IncrementalFolderNames come from configuration (appsettings.json); TargetVersion is
// read from the actual referenced assembly instead (OptimizelyInstalledVersion), so it
// can't drift from the Directory.Packages.props pin the way a duplicated literal could.
    ...
    string TargetVersion)
{
    public static StorefrontMigrationOptions FromConfiguration(StorefrontComponent component, IConfiguration configuration)
    {
        ...
        // Resolved against the app's own base directory, not the process's current
        // working directory - CopyOptimizelySchemaScripts (the .csproj) copies scripts
        // there, and the two don't always match (e.g. `dotnet run` from the project
        // directory).
