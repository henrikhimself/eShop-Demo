    ...
    : ServiceBusQueueConsumer(client, KnownNames.ResourceSellerSubmissionsCancellations, logger)
{
    public override async Task<MessageHandlingResult> HandleMessageAsync(BinaryData body, CancellationToken cancellationToken)
    {
        ...
        {
            // Expected race, not a corrupt message: the simulator may have already
            // resolved (Approved/Rejected) this submission before the cancellation
            // arrived, or never received it at all (queue drained after a restart).
            logger.LogWarning(
