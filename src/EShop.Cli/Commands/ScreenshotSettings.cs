...
internal sealed class ScreenshotSettings : DefaultSettings
{
    ...
    [CommandOption("--login")]
    [Description(
        "Log in via Keycloak before navigating to <URL>, reusing a persistent browser "
            + "profile under tmp/screenshot-profile across invocations (clear that directory to force a fresh login).")]
    public bool Login { get; set; }

    [CommandOption("--username <USERNAME>")]
    [Description("Username for --login. Default: the seeded dev Seller Portal test account.")]
    public string Username { get; set; } = "test-seller";

    [CommandOption("--password <PASSWORD>")]
    [Description("Password for --login. Default: the seeded dev Seller Portal test account.")]
    public string Password { get; set; } = "TestSeller123!";
}
