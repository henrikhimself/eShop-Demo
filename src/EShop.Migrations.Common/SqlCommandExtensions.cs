...
// Extracted from EShop.Migrations.Optimizely/EShop.Migrations.Orchestration: both
// projects independently create a SqlCommand against a connection (and optionally a
// transaction), configure its text/type/parameters, then execute it - this collapses
// that ceremony to one call at each site. Deliberately does NOT cover
// SchemaMigrationLock.TryAcquireAsync: that call needs the SqlCommand alive after
// execution to read back an output ReturnValue parameter, which these
// execute-and-discard helpers don't support - not worth generalizing for one caller.
