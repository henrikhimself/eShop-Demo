# 0013 - Pinned dependency versions, updated only with a 7-day quarantine
...
usually finds and reports a compromised version within its first week of release.
...
By default, the team does not update a dependency to a version less than 7 days
old. The 7-day wait gives the community time to find and report a supply chain
attack in a new version before the team adopts it.

The team makes an exception to the 7-day wait for a very high priority update,
...
- Before an update, the team checks the new version's release date against the
  7-day default wait, unless the update is a very high priority exception.
- The team has not yet automated the 7-day check. The team performs this check by
